...
（4）块时间管理：提供数据块进入模拟内存的时间、访问时间的记录和查询功能，为页面置换算法提供支持，
    当某个内存块被选中时，更新其访问时间。
...
# 16*4B的模拟内存
ram = []
chuck = b'\x00\x00\x00\x00'  # 一个块4字节
...
# TODO：在内存块被换出时通过调用线程的对换方法换出，以便于通知线程更改页表；
# DONE：对内存读写加锁，访问ram都要申请锁，在同一时间线上只允许一个线程进行内存读或写的操作
